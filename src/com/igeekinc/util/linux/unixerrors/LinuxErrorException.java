...
public class LinuxErrorException extends IOException
{
    private static final long serialVersionUID = 4192786548333363815L;
    ...
    public LinuxErrorException(int errorNum)
    {
        super("Unix errno = "+errorNum);
        this.errorNum = errorNum;
    }
    
    public LinuxErrorException(int errorNum, String message)
    {
        super(message + "(errno = "+errorNum+")");
        this.errorNum = errorNum;
    }
    ...
    public static LinuxErrorException getErrorForNumber(int errorNum)
    {
        return getErrorForNumber(errorNum, "", "");
    }
    
    public static LinuxErrorException getErrorForNumber(int errorNum, String message)
    {
        return getErrorForNumber(errorNum, message, "");
    }
    
    public static LinuxErrorException getErrorForNumber(int errorNum, String message, String filePath)
    {
        switch(errorNum)
        {
        case EPERM:
            return new OperationNotPermittedErrorException(message, filePath);
        case ENOENT:
            return new NoSuchFileOrDirectoryErrorException(message, filePath);
        case EIO:
            return new InputOutputErrorException(message, filePath);
        case EACCES:
            return new PermissionDeniedErrorException(message, filePath);
        case EEXIST:
            return new FileExistsErrorException(message, filePath);
        case ENOTDIR:
            return new NotADirectoryErrorException(message, filePath);
        case EISDIR:
            return new IsADirectoryErrorException(message, filePath);
        case EINVAL:
            return new InvalidArgumentErrorException(message, filePath);
        case ENOSPC:
            return new NoSpaceOnDeviceErrorException(message, filePath);
        case ELOOP:
            return new TooManyLevelsOfSymlinkErrorException(message, filePath);
        case ENAMETOOLONG:
            return new NameTooLongErrorException(message, filePath);
        case EDQUOT:
            return new DiscQuotaExceededErrorException(message, filePath);
            
        default:
            if (filePath != null && filePath.length() > 0)
                return new GeneralFileErrorException(errorNum, message, filePath);
            else
                return new LinuxErrorException(errorNum, message);
        }
    }
}
